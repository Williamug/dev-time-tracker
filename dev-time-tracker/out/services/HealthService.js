...
class HealthService {
    ...
    loadConfig() {
        ...
        this.breakReminderInterval = config.get('breakReminderInterval') || 30 * 60;
    ...
    async checkBreakReminder() {
        if (!this.isActive || this.breakReminderInterval <= 0)
            return;
        const now = Date.now();
    ...
    async checkPostureReminder() {
        if (!this.isActive || this.postureReminderInterval <= 0)
            return;
        const now = Date.now();
    ...
    async checkEyeStrainReminder() {
        if (!this.isActive || this.eyeStrainInterval <= 0)
            return;
        const now = Date.now();
