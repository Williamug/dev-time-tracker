...
export class StatusBarManager {
    ...
    public static getInstance(context?: vscode.ExtensionContext): StatusBarManager | null {
        ...
        try {
            if (!StatusBarManager.instance && context) {
                ...
                const disposable = vscode.commands.registerCommand('devtimetracker.togglePomodoro', () => {
    ...
    private updateSessionTimer(): void {
        if (!this.sessionStartTime) {
            console.log('No session start time set');
            return;
        }
        ...
        // For debugging, show seconds
        this.statusBarItems.sessionTimer.text = `$(watch) ${diffMins}m ${diffSecs}s`;
        this.updateTodaySummary(diffMins);
    }
