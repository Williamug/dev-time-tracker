...
// Declare Web Audio API interfaces
...
export class NotificationManager {
  ...
  private readonly soundPresets: Record<Exclude<SoundType, 'none'>, SoundPreset> = {
  ...
  private async initializeAudio(): Promise<void> {
    try {
      ...
      // Preload all sounds
      await Promise.all(
        Object.entries(this.soundPresets).map(async ([name, preset]) => {
          try {
            ...
            if (this.audioContext) {
              ...
              this.sounds.set(name as SoundType, audioBuffer);
            }
          } catch (error) {
  ...
  private async loadSound(name: SoundType, preset: SoundPreset): Promise<void> {
  ...
  private playSound(type: SoundType = 'default'): void {
  ...
  private generateWav(preset: SoundPreset): string {
    const {
      duration,
      type,
      freq,
    ...
    for (let i = 0; i < samples; i++) {
      ...
      else if (t < attack + decay) value *= 1 - (1 - sustain) * ((t - attack) / decay);
      else if (t > duration - release) value *= 1 - (t - (duration - release)) / release;
